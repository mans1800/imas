...
function splitDateDbCH($date, $sp = "-") {
    list($yy, $mm, $dd) = preg_split("[/|-]", $date);
    return $yy . "$sp" . $mm . "$sp" . $dd;
}
...
function getconstros($roomtype_id,$start_date)
{
   $CI =& get_instance();
   $CI->load->model('mo_tb_roomtype');
   return $CI->mo_tb_roomtype->get_check_type($roomtype_id,$start_date);
